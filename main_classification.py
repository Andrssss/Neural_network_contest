# EZEN A VERZIÓN - validation_ratio     = 0.014

validation_ratio = 0.1  # -> 50 kép
...
dropout = 0, 2  # mmodellben is át kell irni
patience = 100

configurations = [
    (150, 8, 1, "MobileNetV2Custom", 0.003),
    # (150, 16, 1, "MobileNetV2Custom",0.003),
    # (150, 32, 1, "MobileNetV2Custom",0.001),
    # (100, 64, 1, "MobileNetV2Custom_0",0.0005),
    # (100, 128, 1, "MobileNetV2Custom_0",0.0001),

    (150, 8, 1, "MobileNetV2Custom_0", 0.003),
...
train_image_list, train_image_ids, validate_image_list, validate_image_ids, test_image_list, test_image_ids, data_array = initialize_data(
    output_dir, validation_folder, test_folder, label_file, validation_ratio, kerekitsen_labeleket)
...
# Használjuk a súlyokat a veszteségfüggvényben
...
for num_epochs, train_batch_size, fel_le_kerekit, model_neve, default_lr in configurations:
    ...
    cur_acc = 0.0
    if (previous_config != None and num_epochs > previous_config[0] and train_batch_size == previous_config[
        1] and fel_le_kerekit == previous_config[2] and model_neve == previous_config[3]):
        ...
        if validate_image_tensors is not None:
            ...
            val_dataset = CustomImageDataset(images=validate_image_tensors, image_ids=validate_image_ids,
        ...
        if model_neve == "MobileNetV2Custom_0":  # Epoch : 0–50 , Batch Size : 32 vagy 64
            ...
            criterion = criterion = nn.CrossEntropyLoss(weight=scaled_weights.to(device))
            # criterion = nn.CrossEntropyLoss(label_smoothing=0.1)
            logging.info("criterion = label_smoothing_loss")
            ...
            # criterion = label_smoothing_loss
            criterion = criterion = nn.CrossEntropyLoss(weight=scaled_weights.to(device))
            logging.info("criterion = CrossEntropyLoss")
            ...
            # criterion = label_smoothing_loss
            criterion = criterion = nn.CrossEntropyLoss(label_smoothing=0.1, weight=scaled_weights.to(device))
            logging.info("criterion = criterion = nn.CrossEntropyLoss(label_smoothing=0.1,weight=scaled_weig")
            ...
            logging.info("optimizer = optim.Adam(model.parameters(), lr=0.001)")
            logging.info("scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=20, gamma=0.05)")
            logging.info("criterion = criterion = nn.CrossEntropyLoss(label_smoothing=0.1,weight=scaled_weig")
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if validate_image_tensors is not None and epoch > 20:
            model.eval()
            val_loss = 0.0
            correct_val = 0
            total_val = 0
        ...
        if val_accuracy > 15 and train_accuracy > 75:
            evaluate_model(model, test_image_tensors, test_image_ids, label_map, dev, epoch, train_batch_size,
